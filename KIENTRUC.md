...
| **Full sync khi máy kia đã học mà chưa sync** | Hai bản rẽ nhánh; Anki KHÔNG trộn được, chỉ cho chọn một bên thắng | CỨU ĐƯỢC (làm thật 23/08): chụp `word→tag` ra `backups/` TRƯỚC, cho bên có LƯỢT ÔN thắng, rồi áp lại bản đồ + `build_subdecks.py`. Cây chạy lại được, lượt ôn thì không |
