...
def add_note(fields, deck=PLURAL_DECK, tags=None, model=PLURAL_MODEL):
    ...
    `model` giữ dạng tham số (mặc định RU_Plural) dù mảng này nay chỉ còn MỘT
    loại thẻ: chỗ gọi duy nhất truyền tên model vào là chỗ dễ thấy nhất khi có
    loại thẻ thứ hai, còn hằng số nhúng cứng thì phải đi tìm.
    """
    try:
