...
👤 **`PHIENBAN.md` là file DUY NHẤT viết cho USER — mọi file khác viết cho người sửa code.** Deploy xong mà có thay đổi **user cảm nhận được** (nút mới, lỗi họ từng gặp đã sửa, thẻ hiện khác đi) thì thêm một mục: số hiệu `vX.Y.Z` + vài gạch đầu dòng, **ngôn ngữ thường, không thuật ngữ** (trần bản/mục: `soat_nguong.json`, cửa S14 canh). Dọn code / đổi cấu trúc / thêm cửa soát thì **KHÔNG ghi** — user không thấy chúng. (QD-07)
