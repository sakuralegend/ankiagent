...
♻️ **SINH PHẢI BẰNG TỬ (QD-29) — luật giữ cho repo sống lâu.** `QUYETDINH.md` có **sức chứa CỐ ĐỊNH**, cửa **S20** đếm thật. Thêm một dòng ⇒ phải **bỏ một dòng**; bỏ thì rẻ vì **kho lưu trữ là `git log`** — `git log --grep QD-nn` ra nguyên văn, dài gấp 6–16 lần dòng bảng (đo 04/08, che phủ 100%). **Đừng đẻ file lưu trữ song song** — đã thử sáng 04/08 rồi xoá, nó tái phạm đúng lỗi `CHANGELOG.md` (QD-06). 🔴 **CẤM nén chữ để nhét thêm** — nén là cắt phần *vì sao*, thứ duy nhất còn dùng được khi gặp tình huống mới. Phép thử cho một mục ĐƯỢC CHẾT: *"tình huống nào cần biết điều này, và lúc đó người ta CHẮC CHẮN đang mở file nào / đọc lời báo lỗi nào?"* — có chỗ như thế ⇒ chép lý do vào đúng đó rồi cho dòng rời sổ; không có ⇒ giữ.
...
- File code quá trần dòng (số ở `soat_nguong.json`, cửa S13 canh): trần ghi nợ → ghi `SONO.md` + mốc vào `da_ghi_no`; trần tách → tách trước khi thêm.
